...
void merge(int* first_array, int n_first, int* second_array, int n_second, int* merged_array)
{
    ...
    { merged_array[k]=first_array[i]; i++
    ; }
    else
    { merged_array[k]=second_array[j]; j++
    ; }
    k++;
    }
    // fill up any elements left over from above
    while (i < nL)
    { merged_array[k]=first_array[i]; i++; k++
    ; }
    while (j < nR)
    merged_array[k]=first_array[i] ; j++; k++;
